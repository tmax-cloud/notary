...
if [ -z $SERVER_IMG ] || [ -z $SIGNER_IMG ] || [ -z $MYSQL_IMG ]; then
...
docker build -t $MYSQL_IMG:$NOTARY_VERSION -f custom.mysql.Dockerfile .

docker push $SERVER_IMG:$NOTARY_VERSION
docker push $SIGNER_IMG:$NOTARY_VERSION
docker push $MYSQL_IMG:$NOTARY_VERSION
